...
var EPHChart = function(opts, data_) {  
  ...
  var calc = function(dataSet, initalData_) {
    ...
    if (dataSet) {
      for (var d in dataSet) {
        ...
        switch (opts.startBar) {
          case 'currentMinute':
            t = moment(dataSet[d].timestamp).minute();
            break;
          default:
            t = moment(dataSet[d].timestamp).hour();
        }            
        initalData_[t].value++;              
      }         
    }
    ...
    if (scaleFactor > 1) {
      for (var i = 0, ix = initalData_.length; i < ix; ++i) {
        if (initalData_[i].value > 0) {
          var old = initalData_[i].value;
          initalData_[i].value = Math.round(initalData_[i].value / scaleFactor);                
        }
      }  
    }              
    if (dataSet) {  
