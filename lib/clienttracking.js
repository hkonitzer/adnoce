...
* @return false if no sessionID or url (referer) was provided, true otherwise
*/
var pushTrackingData = function(sessionID, requestHeaders, additionalData, callback) {  
  ...
  if (additionalData.timestamp) newTrackingData.timestamp = additionalData.timestamp;
  ...
  newTrackingData.save(function(err) {
    if (err) {
      ...
      updateSessionData(sessionID, null, additionalData, function(error, updata){
        ...
        if (typeof(callback) === 'function') callback(error, mergeJSObjects(newTrackingData.toJSON(), updata));
      });
...
var updateSessionData = function(sessionID, persistentSessionID, additionalData, callback) {
  ...
  if (typeof(additionalData) === 'object' && additionalData !== null) updateObject = additionalData; else updateObject = {};
  ...
  models.AdnoceSession.findOneAndUpdate({ sessionId: sessionID }, updateObject, { upsert: true }, function(err, doc) {
    if (err) log.error(err);    
      ...
      if (additionalData) answer.adnocetype = additionalData.adnocetype || 100;
      else answer.adnocetype = 100;
...
*/
var addEvent = function(type, name, sessionId, additionalData, eventDate, callback) {    
  var newEvent = new models.AdnoceEvent();
  newEvent.adnocetype = type;  
  newEvent.sessionId = sessionId;
  if (name) newEvent.eventname = name;
  if (additionalData) {
    newEvent.data = additionalData;
  }  
  if (typeof(eventDate) === 'object') {
    newEvent.timestamp = eventDate;    
  } else if (typeof(eventDate) === 'function') callback = eventDate;
  newEvent.save(function(error) {
    if (error === null && io) io.emit('trackingdata', newEvent);    
...
exports.mergeJSObjects = mergeJSObjects;