...
var sendMapReduceCommand = function(commandObject, callback) {
  /*
  var command = {
      mapreduce: "serverusagelogs",
      //the name of the collection we are map-reducing *note .mongoose automatically appends an 's' to the model name within mongoDB
      //query: { 'active.end' : { $gt: new Date() } }, //I've included this as an example of how to query for parameters outside of the map-reduced variable
      map: companyMap.toString(),
      //a function we'll define next for mapping
      reduce: companyReduce.toString(),
      //a function we'll define next for reducing         
      out: _serverusagelogsDailyReduceModelName //the collection that will contain the map-reduce results *note, this must be a different collection than the map-reduce input
   };
   */
...
"if (url.indexOf('presse')>-1) print('PRESSE '+url+' = '+emitObj.year+' / '+emitObj.month + ' / '+emitObj.day+ ' / '+emitObj.hour);"+
"emit(url, emitObj);"+
"}"
;
