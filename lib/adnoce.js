...
exports.setDatabase = function(databaseConfiguration) {
  db.connect('mongodb://'+databaseConfiguration.host+'/'+databaseConfiguration.name, function(){  
    log.info('adnoce creating database connection to "%s" on host "%s", status: %s', databaseConfiguration.host, databaseConfiguration.name, db.connection.readyState);  
...
var socketConnection = function(socket_) {
    ...
    log.info('adnoce socket connected to endpoint "%s"', socket.flags.endpoint);
}
